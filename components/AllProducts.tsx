...
export default function AllProducts({ products }: Products) {
  ...
  return (
    <div className="mz_container-body px-3">
      ...
      <div
        className="w-full gap-x-1 gap-y-3 md:gap-3 items-center grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center"
        // className={`gap-x-1 gap-y-3 md:gap-3  ${formattedProducts.length === 1 ? 'w-1/2 mx-auto' : 'w-full grid gap-4 grid-cols-1 xs:grid-cols-2 md:grid-cols-3'}`}
      >
        {/* {[...Array(4)].map((_, i) => ( */}
        {/* <div key={i}> */}
